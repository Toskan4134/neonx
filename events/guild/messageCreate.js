...
module.exports = async (client, message) => {
  ...
  if (cmd == null || cmd.length == 0) {
    if (message.mentions.has(client.user) || mPrefix.includes(client.user.id)) {
      message.reply({ embeds: [new Discord.MessageEmbed().setColor(ee.color).setFooter({ text: ee.footertext, iconURL: ee.footericon }).setTitle(`:thumbsup: **Mi prefix aquí is \`${prefix}\`**`)] })
    }
    return;
  }
  ...
  if (command) {
    if (!command.activated) {
      ...
      if (settings.ownerIDS.includes(message.author.id)) {
        await msg.react("👑")
      ...
      if (command.minargs && command.minargs > 0 && args.length < command.minargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setDescription(command.argsmissing_message && command.argsmissing_message.trim().length > 0 ? command.argsmissing_message : command.usage ? "Uso: `" + command.usage + "`" : "`Uso incorrecto del comando`")]
      ...
      if (command.maxargs && command.maxargs > 0 && args.length > command.maxargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setFooter({ text: ee.footertext, iconURL: ee.footericon })
            .setTitle(":x: Uso incorrecto del comando")
            .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argstoomany_message : command.usage ? "Uso: `" + command.usage + "`" : "`Uso incorrecto del comando`")]
      ...
      if (command.maxplusargs && command.maxplusargs > 0 && args.join(" ").split("++").filter(Boolean).length > command.maxplusargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setFooter({ text: ee.footertext, iconURL: ee.footericon })
            .setTitle(":x: Uso incorrecto del comando")
            .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argsmissing_message : command.usage ? "Uso: `" + command.usage + "`" : "`Uso incorrecto del comando`")]
