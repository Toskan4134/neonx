...
module.exports.deactivatedCommand = deactivatedCommand;
module.exports.replacemsg = replacedefaultmessages;
module.exports.ticketOptions = ticketOptions;
module.exports.guildMemberStats = guildMemberStats;
...
function replacedefaultmessages(text, o = {}) {
  if (!text || text == undefined || text == null) throw "No se agregó texto para el mensaje de reemplazo predeterminado en el primer parámetro";
...
/**
 * 
 * @param {*} cmd Comando a comprobar si está desactivado o no
 * @param {*} id Id del solicitante del comando
 * @returns 
 */
function deactivatedCommand(cmd, id) {
  return cmd.category === "Deactivated" && !settings.ownerIDS.includes(id)
}
...
function parseMilliseconds(milliseconds) {
  if (typeof milliseconds !== 'number') {
    throw new TypeError('El valor introducido no es un número');
  }
...
function GetUser(message, arg) {
  var errormessage = ":x: Usuario no encontrado...";
...
function GetRole(message, arg) {
  var errormessage = ":x: Rol no encontrado...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if (!client || !message) return reject("CLIENT IS NOT DEFINED")
    if (!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
...
function createBar(total, current, size = 25, line = "▬", slider = "🔷") {
  try {
    if (!total) throw "Falta el tiempo máximo";
...
async function swap_pages(client, message, description, TITLE) {
  ...
  if (embeds.length === 0) return message.channel.send({
    embeds: [new MessageEmbed()
      .setTitle(`${emoji.msg.ERROR} No se ha añadido contenido para la función de cambio de página`)
  ...
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("833802907509719130").setLabel("Retroceder")
  let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("🏠").setLabel("Inicio")
  let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('832598861813776394').setLabel("Avanzar")
  ...
  let swapmsg = await message.channel.send({
    content: `***Haz click en los __Botones__ para cambiar de página***`,
    embeds: [embeds[0]],
  ...
  collector.on('collect', async b => {
    if (b.user.id !== message.author.id)
      return b.reply(`<:no:833101993668771842> **Solo el que ha escrito ${prefix}help puede reaccionar**`, true)
    //page forward
    if (b.customId == "1") {
 ...
 */
function change_status(client, type, text) {
  try {
...
/**
 * 
 * @param {MessageButtonOptions} type 
 */
function ticketOptions(type, member) {
  var emoji = "";
  var staffRole = settings.StaffID;
  var msg = "Bienvenido/a al soporte de **__Neon Servers__**, <@" + member + ">\n" +
    "El equipo de Staff <@&" + settings.StaffID + "> estará contigo en seguida\n" +
    "__Recuerda rellenar la plantilla para poder agilizar el proceso__\n" +
    "Plantilla:"
  var embed = "**- ID Ingame / URL Steam:**\n" +
    "**- Tu banda o mafia:**\n" +
    "**- Motivo del reporte: (Descripción)**\n" +
    "**- Pruebas: (Si las hay y son necesarias)**\n" +
    "**- Reclamación: (Que es lo que pides con este reporte)**"
  switch (type) {
    case "Bugs-Errores":
      emoji = "🐞";
      break;
    ...
    case "Mafias-Bandas":
      emoji = "🕵️";
      staffRole = settings.tickets.encargadoIlegales;
      msg = "Bienvenido/a al soporte de **__Neon Servers__**, <@" + member + ">\n" +
        "El equipo de Staff <@&" + settings.tickets.encargadoIlegales + "> estará contigo en seguida\n" +
        "__Recuerda rellenar la plantilla para poder agilizar el proceso__\n" +
        "Plantilla:"

      break;
    case "Facciones-Legales":
      emoji = "👮";
      staffRole = settings.tickets.encargadoLegales;
      msg = "Bienvenido/a al soporte de **__Neon Servers__**, <@" + member + ">\n" +
        "El equipo de Staff <@&" + settings.tickets.encargadoLegales + "> estará contigo en seguida\n" +
        "__Recuerda rellenar la plantilla para poder agilizar el proceso__\n" +
        "Plantilla:"
      break;
    case "CKs-Banneos":
      emoji = "💀"
      staffRole = settings.tickets.encargadoCKs;
      msg = "Bienvenido/a al soporte de **__Neon Servers__**, <@" + member + ">\n" +
        "El equipo de Staff <@&" + settings.tickets.encargadoCKs + "> estará contigo en seguida\n" +
        "__Recuerda rellenar la plantilla para poder agilizar el proceso__\n" +
        "Plantilla:"
      break;
    case "Contacto-Fundadores":
      emoji = "💠"
      staffRole = settings.founderID;
      msg = "Bienvenido/a al soporte de **__Neon Servers__**, <@" + member + ">\n" +
        "El equipo de Staff <@&" + settings.founderID + "> estará contigo en seguida\n" +
        "__Recuerda rellenar la plantilla para poder agilizar el proceso__\n" +
        "Plantilla:"
      break;
  }
  return { staffRole, emoji, msg, embed }
}


function guildMemberStats(channel, message) {
  try {
    if (channel.name === message) return;
